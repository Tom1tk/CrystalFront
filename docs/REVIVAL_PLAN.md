...
- **H5 — `chooseBuildPosition` fallback-offset `dx` unmirrored.** CONFIRMED, and the `dxSign = isBlue ? 1 : -1` fix is now PROVEN exactly mirror-equivariant via property test (C.1 #15: 0/9 zone-combo violations over a full 6000-tick match, vs violations in 5/9 combos before the fix, including at tick 0). The fix is **ready to land** but is **blocked on H7** — landing it alone reproduces the C.1 #12 overshoot (10-20-0, red 67%, C.1 #16). All other `isBlue ? ... : ...` branches and static map constants in `actionSpace.ts`/`observation.ts`/`gameBalance.ts` were swept and check out as symmetric.
- ~~**H6 — Multiple compensating asymmetries**~~ — PROVEN via C.1 #16 (no longer just an overshoot-signature hypothesis): with H5 fixed AND property-tested clean (C.1 #14/#15 cover `tick()` and `chooseBuildPosition` exhaustively), the matrix is STILL 67% red. By elimination, ≥1 more asymmetry exists outside both of those. Superseded by H7, which narrows *where*.
- **H7 — Bot-decision-layer asymmetry (STILL OPEN).** `tick()` (C.1 #14), `chooseBuildPosition` (C.1 #15), and `resolveTargetZone`/`findNode` (C.1 #17, the cross-color/same-state audit — the *valid* and now-*complete* form of C.3 #7, see C.1 #20) are all clean (mod RNG-divergence-explainable noise, #18, and the unreachable dead code, #19). H6's "other asymmetry" must therefore live in the parts of the bot-decision layer **no equivariance audit has covered yet**: **`MediumRushBot.step()`'s own logic** (the `safeNodes`/`xNorm` filter, `workerTarget`, tick-gated counters — `headless/src/bots/mediumRushBot.ts`) and **`buildObservation()`'s per-entity/per-node features** (`xNorm`, `isContested`, etc. — only the *global* scalars were checked in #18, `headless/src/observation.ts`). The originally-specced "full stateful lockstep" test (`mirrorState`-based, two trajectories) is **abandoned as methodologically invalid** (C.1 #20 — `mirrorState` decouples `playerId/.color` from `ownerId`'s actual entity-side, breaking any `match.players`-lookup-based `isBlue`). **Caveat added 2026-06-10 (independent audit): the evidence that H7 exists AT ALL is weaker than C.1 #16's "proven" wording implies.** The post-H5 result (10-20-0) spans only 30 seeds: under a true-50% game, P(blue wins ≤ 10 of 30) ≈ 0.049 — suggestive, not proof — and the #12/#16 "byte-identical replications" reused the *same* seeds 1-30, which proves per-seed determinism but adds zero statistical power about the cross-seed mean. (Pre-H5 22-8-0 is p ≈ 0.008, stronger — but H5 is a confirmed real asymmetry that explains it.) So the residual skew may be sampling noise and **H5 alone may already pass criterion 4**. Test that first (C.3 #8, fresh seeds) before building ANY new audit infrastructure. If the skew replicates, C.3 #9 (symmetric-by-construction match) supersedes the previously-proposed ticks-0-52 per-entity audit: the 0-52 window is blind to asymmetries that only manifest in combat-era decisions (`MediumRushBot`'s `attack_move` only starts at tick ≥ 200), while #9 extends the guaranteed-clean comparison window to the entire match. See C.4 "Path forward".
...
8. **Fresh-seed replication of the post-H5 matrix (~30 min) — RUN THIS FIRST.** Apply the H5 fix (do not land yet) and run the rush_medium mirror matrix on **100 fresh seeds (31–130)** via `_matrixAudit.ts` (parameterize the seed range). Rationale: every matrix so far used seeds 1–30, and 10-20-0 is only p ≈ 0.049 under a fair game — "H7 exists" currently rests on that. Decision rule: **if the 100-seed blue winrate lands in 40–60% → there is no H7.** Land H5 alone (R1/R2/R7/R10), re-run the rush_weak/macro mirror pairings and the criteria-1/2/3/5 matrix, close criterion 4 and this appendix. If it lands outside 40–60% (≤ 40 or ≥ 60 wins of 100 is p < 0.03 one-sided) → the residual asymmetry is real; proceed to #9.

9. **Symmetric-by-construction mirror match (spawn determinization, ~1–2 h) — only if #8 confirms the skew.** Per C.1 #21, `spawnOutside` (`matchEngine.ts:1492`) is the engine's ONLY per-seed-RNG gameplay consumer. For the experiment (not a landable fix), replace its isotropic angle draw with a deterministic mirror-symmetric one — e.g. `angle = parent.x < match.mapWidth/2 ? 0 : Math.PI` (spawn toward map centre; equivariant because `cos` flips sign with side and the `sin` term is 0) — and keep the H5 fix applied. By C.1 #13 (symmetric init) + #14 (tick equivariant; noise floor bounded ≤ 0.5px and non-cascading over 6000 ticks), the only remaining desymmetrizers over an **entire match** are then bot-decision-layer asymmetries — i.e. exactly H7. Outcomes: **(a)** matrix → ~50/50 (or mass timeout-draws via the seed coin flip) ⇒ no H7 function bug exists; the residual skew was spawn-RNG noise interacting with the seed set; land H5 and decide separately whether a symmetric spawn rule is worth landing. **(b)** matrix still skewed ⇒ H7 definitively exists AND becomes trivially localizable: re-run `_stateDivergenceAudit.ts` (and its per-entity extension if needed) on one skewed seed — with spawn noise eliminated, the first diverging tick/feature directly fingers the asymmetric code path at ANY tick of the match, uncontaminated by RNG.
