...
**Status:** Phases 0–2 and Task 3.1 are complete (see Appendix B for per-task status and deviation footnotes). Task 3.2's first two curriculum-run attempts (Iterations 21/23, v0.5.11/12-ML) both cascaded to 0% via an entropy-runaway absorbing state that curriculum regression couldn't escape (Iteration 24 diagnosis). Iteration 25 (2026-06-14, v0.5.13-ML) implemented both proposed fixes — `ent_coef` 0.02→0.01 and an in-memory stage-entry snapshot rollback on regression — smoke-tested clean on GPU. Relaunch attempt #3 (Iteration 26) confirmed both fixes work (no absorbing collapse, entropy bounded, `3a_rm` reached for the first time, twice) but surfaced a new "frontier oscillation" — a recoverable but unproductive `3a_rw`↔`3a_rwm` cycle that never progresses past `3a_rm`. STOPPED at update 2231 for a design pause; a `3a_rm` pre-placed-skirmisher scaffold fix is proposed (Iteration 26), pending sign-off before relaunch #4. *(Original status "Ready to implement — nothing started" superseded by the 2026-06-12 audit.)*
...
- The existing 20-stage `CURRICULUM` table is the starting point, **now that its knobs actually work**. *(Correction, 2026-06-12 audit: this plan originally said "17-stage"; the code table in `training/ppo/train.py` has 20 entries — 0a, 0b, 0b5, 0c, day5, 1a, 1b, 2a, 2a5, 2a6, 2b, 3a, 3a_rw, 3a_rwm, 3a_rm_3k, 3a1, 3a2, 3a5, 3b, 4.)* Expect the early micro-stages (0b5, 0c, day5) to clear much faster than history suggests — or to behave differently than the diary claims (R5). Trust the new logs only.
...
**Pre-flight findings (2026-06-12 implementation audit — close GAP-1 before launching; GAP-2/3 belong in the same pre-flight commit):**

- **GAP-1 (launch blocker): `train.py`'s PPO update loop has no `isfinite(grad_norm)` skip-guard.** `training/ppo/train.py` (~lines 857–860) runs `loss.backward()` → `clip_grad_norm_` → `optimizer.step()` unconditionally. Task 3.1 established that on this GPU stack (ROCm 7.2 / torch 2.12.0), `clip_grad_norm_` occasionally returns `inf` *even with* the `policy.py` LayerNorm fix in place (aggregate-norm overflow with no individual non-finite param) — and an unguarded `inf` total-norm yields `clip_coef=0` → `inf*0=nan` weight corruption on `step()`. In a ~1-week, 20M-decision run a single occurrence silently poisons everything after it. Port the exact pattern from `training/bc_pretrain.py` (lines ~251–253 actor loop, ~298–300 critic loop): `if not torch.isfinite(grad_norm): optimizer.zero_grad(); continue`. (The RND optimizer at ~867–870 has no clipping at all, but RND is default-off (`rnd_coef=0.0`) and deprecated per Task 0.7 — leave it.)
- **GAP-2: Task 0.4 step 2's `botCrashCount` warning in `train.py` was never implemented.** The runner half is done (`stdioVecRunner.ts` surfaces `info.botCrashCount` on done steps) and `crystalfront_vec_env.py` passes the full info dict through — but nothing in `train.py` reads it, so stop-the-line condition 5 above *cannot currently fire*. Add the warning print where done-step infos are processed. Appendix B row 0.4 is downgraded ✅→⚠️ until this lands.
- **GAP-3: realised-config logging (second note above) has no code support.** `nextEpisodeConfig` reaches `train.py`'s info dicts (Task 0.2's plumbing is intact end-to-end) but is never printed. Without a stage-entry print, stop-the-line condition 4 requires manual log archaeology mid-run. Print the first done-step's `info["nextEpisodeConfig"]` alongside the stage definition after every stage entry/env rebuild (and once at run start).

**Audit observations (no action required — recorded so the run operator isn't surprised):**

- Stage `0a` at k=8 is a near-free win: the Task 2.4 smoke (Iteration 16) showed `0a` episodes ending after ~1 decision — the pre-placed barracks + 2 skirmishers kill the 50 HP crystal regardless of what the agent does. Expect near-instant promotion with ~zero learning signal; the meaningful "vs idle ~100%" pipeline check is `0b`/`0b5`/`0c`, where the agent must train units itself.
- Worst-case sum of all stage `max_steps` budgets ≈ 22.75M decisions > `total_timesteps` 20M. If many stages run to their caps, the run ends inside (or before) stage 4 (league) without exhausting that stage's budget. This is the `total_timesteps` cap working as intended, not a crash.
- Verified clean for this run (2026-06-12): BC-checkpoint resume path is correct — `train.py` (~lines 485–489) deliberately skips optimizer-state restore for `update=0` checkpoints, so `bc_warmup_v05.pt` loads as a warm-start without BC's Adam momentum, and `start_update=1` means the Task 2.3 schedule-compression warning correctly stays silent; the shared `LayerNorm` fix in `policy.py` covers PPO (same `CrystalFrontAgent`); forcing (`action_forcing_scale=0.0`) and RND (`rnd_coef=0.0`) are both default-off per Task 0.7; noop%/action-mix and entropy logging already exist in `train.py`, so stop-the-line conditions 2–3 are monitorable from the console/TensorBoard; `docs/eval_v0.3.2-ML.txt` exists for Task 3.3's no-regression gate; `python -m training.test_env` passes (dist fresh, reward scale confirmed).
- Untracked working-tree leftovers: `headless/src/debugMirror.ts` (Appendix C-era debug script) and `docs/SHIPPING_AND_V040_PLAN.md`. Neither is part of any commit — decide to commit or delete them with the pre-flight commit so the long run starts from a clean tree.
...
| 3.1 | Re-record BC demos (k=8, expert fixes) | BC sanity gates | ⚠️ |
...
**Task 2.4 DONE (v0.5.8-ML, 2026-06-11, Iteration 16):** in `training/ppo/train.py`, replaced the immediate in-loop env rebuilds on curriculum promotion/regression with a deferred `pending_stage_change` flag (0 / +1 / -1, initialized alongside `cur_stage`/`stage_step_start`). The win-window check (still inside rollout collection) now only sets `pending_stage_change` and prints "CURRICULUM COMPLETE" if the league stage is reached — it no longer touches `cur_stage_idx`, `cur_stage`, `vec_envs`, or `obs_list`/`info_list` directly. A new block placed after the league step (still inside the `for update in range(start_update, num_updates+1)` loop, just before "final save") applies the deferred change once per update: bumps `cur_stage_idx`, swaps `cur_stage`, resets `stage_step_start`, prints the PROMOTE/REGRESS line, logs `curriculum/stage`, closes and rebuilds `vec_envs` via `_resolve_env_params()`/`_build_vec_envs()`, re-resets all envs, and — new beyond the literal plan text — resets `episode_rewards`/`episode_lengths` for **all** envs (not just the env that triggered `done`), since every env's underlying game instance is replaced by the rebuild. This guarantees a single `RolloutBuffer`/GAE computation never spans two curriculum stages/configs. **Verify:** ran a throwaway smoke test (temporarily set `WIN_WINDOW=8` and stage `0a`'s `promotion_threshold=0.0`/`max_ticks=80`, both reverted immediately after — final diff is the 29/30-line curriculum-only change shown above) with `--curriculum --curriculum_stage 0 --num_envs 4 --vec_size 4 --decision_interval 8 --device cpu --no-compile_agent --num_steps 16 --total_timesteps 640` (10 updates). Stage `0a` episodes terminated after 1 decision each (pre-placed skirmishers + idle opponent + 50 HP crystal — expected, independent of this change); `window_episodes` crossed 8 eight times during update 1's rollout collection (8 window-report lines, all `update=1`), the 8th crossing set `pending_stage_change=+1`. The `*** CURRICULUM PROMOTE → stage 0b (map=800, opp=idle) ***` line printed **after** update 1's last window-report line and **before** any update-2 output — i.e. between update logs, exactly as the plan specifies. Updates 2-10 then ran in stage `0b` (no episodes completed within the remaining budget, so no further window-report lines — expected) and the loop reached `num_updates=10` cleanly, printing `Training complete. Final checkpoint: .../final.pt`. No exceptions, no negative-LR or empty-range edge cases recurred. `npm test` 470/470 (no TS files touched; R6 regression check). No `BALANCE_HISTORY` entry needed (trainer-only change, no engine/reward/replay effect). Version bumped to `0.5.8-ML` (R1). **Phase 2 Task 2.5 (BC label alignment) is next.**

**Task 2.5 DONE (v0.5.9-ML, 2026-06-11, Iteration 17):** in `training/bc_pretrain.py`'s `collect_demonstrations()`, fixed the off-by-one between observations and demo-action labels, exactly per this section's diff. Old loop read `demo_action = info.get("demoAction", 0)` (the action produced by the *previous* `env.step()`, or the stale/absent value from `env.reset()`'s info on the first iteration) **before** appending it alongside the *current* `obs`, then stepped — so `ep_acts[i]` held the action that produced `ep_obs[i]` (action a_{t-1} paired with observation s_t), one step stale relative to what BC needs (the action the expert *would take from* `ep_obs[i]`). New loop appends `obs` first, then steps, then reads `info.get("demoAction", 0)` from the result of *that* step — so `ep_acts[i]` is now the action the demo bot took starting from `ep_obs[i]` (correct (s_t, a_t) pairing). `ep_rews`/`done` logic, the noop-subsample (`cfg.noop_keep_frac`), and the discounted-return critic targets (`GAMMA=0.995` backward pass) are byte-for-byte unchanged — only the 6-line collection loop body changed (no other lines in the file touched). `python3 -c "import training.bc_pretrain"` clean. **Verify:** ran `python3 -m training.bc_pretrain --episodes 2 --epochs 1 --device cpu --max_ticks 80 --output /tmp/bc_smoke_2_5.pt` (rush vs idle, k=1 — `CrystalFrontEnv` has no `decision_interval` param, unaffected by Task 2.1): collected 160 transitions from 2 kept episodes (80 ticks/episode), noop-subsampled to 13 (6 non-noop + 7 noop, matching `max(1, int(154*0.05))=7`), critic targets `min=-1.0 mean=-0.8 max=-0.7` (consistent with the ±1.0 terminal scale from Task 2.2), 1 actor epoch + 3 critic epochs ran without error, checkpoint saved. Also re-ran `python3 -m training.test_config_persistence` (Phase 2 exit criteria explicitly calls this out as "the runner changed — re-run it") — `PASS: config overrides persisted across 120 steps (episodes_done=[2, 2])`. `npm test` 470/470 (no TS files touched). No `BALANCE_HISTORY` entry needed (Python-only training-pipeline fix, no engine/reward/replay effect). Version bumped to `0.5.9-ML` (R1). **All five Phase 2 tasks (2.1-2.5) are now ✅ in Appendix B — Phase 2 task P2 (diary + `ML_AGENT.md` §4/§8 sync + handoff checklist, the phase-boundary exit task) is next.**

**Task P2/Phase 2 EXIT (v0.5.9-ML, 2026-06-11, Iteration 18):** audited `docs/ML_AGENT.md` §4 (already current from Task 2.2's in-commit update) and §8 against all of Phase 2's changes, making 5 targeted edits — §8.4 (decision-vs-tick throughput note for `decision_interval=8`), §8.5 (negative-update clamp + new `--anneal_lr` schedule-compression warning, both Task 2.3), §8.9 (hyperparameter table: new `--decision_interval` row, updated `--num_steps`/`--num_minibatches`/`--gamma`), §8.10 (replaced the stale "ep_len locked at 6000 → drawing" stop condition, which no longer applies since Task 2.2 removed the draw outcome, with a `diagnostics/timeout_rate`/`game/episode_length_ticks` check), and §10.1 (`game/episode_terminal_return` magnitude `±100`→`±1.0` per Task 2.2). Two pre-existing, out-of-scope staleness items were identified and deliberately left as-is (documented in `docs/ML_BOT_ACTION_PLAN.md` Iteration 18's "what NOT to waste time on"): §5's curriculum-stage table (missing `3a_rw`/`3a1`/`3a2`/`3a5`, pre-dates Phase 2) and §10.1's `game/episode_terminal_return` tag-NAME mismatch (actual logged tag is `rewards/terminal_reward_mean`, also pre-dates Phase 2). Rewrote `docs/ML_BOT_ACTION_PLAN.md`'s "Current handoff state" ("Where we are", "What was learned" item 3, "Open commitments", Key commands comment) to mark Phase 2 fully ✅ COMPLETE and point at Phase 3 Task 3.1. Wrote the full Iteration 18 diary entry including the mandatory **Reflections (a/b/c)** subsection (R10 phase-boundary requirement) — flags one confirmed Phase 3 Task 3.1 prerequisite: `CrystalFrontEnv` (BC path) has no `decision_interval` constructor param (confirmed via grep), so Task 3.1 must add it before re-recording BC demos at k=8. `npm test` 470/470 (no `headless/src` files touched); `test_config_persistence` already re-verified PASS in Iteration 17, no runner changes since. No version bump (R1 n/a — docs only, per the Iteration 12/Phase-1-EXIT precedent). **Phase 2 exit criteria (line 341) all satisfied. Phase 2 is COMPLETE. Phase 3 Task 3.1 (re-record BC demos, add `decision_interval=8` to `CrystalFrontEnv`) is next.**

**⚠️ Task 3.1 deviation (v0.5.10-ML, 2026-06-11, Iteration 19):** done with one literal-gate miss, documented honestly. Added `decision_interval` to `CrystalFrontEnv` (Task 2.1-style plumbing, per Iteration 18's flagged prerequisite), set `bc_pretrain.py`'s `Config` to the full-game k=8 MDP (`map_width=0`/`crystal_health=0` → `DEFAULT_CONFIG` 6000px/1000HP, `max_ticks=6000`, `decision_interval=8`, `GAMMA=0.99` to match Task 2.3), and re-ran the 1000-episode `rush` vs `idle` collection. The first full run produced `loss=nan`/`mse=nan` for **every** epoch. Root cause: `nn.LayerNorm`'s CUDA backward kernel on ROCm 7.2/torch 2.12.0 corrupts roughly half of `grad_weight`/`grad_bias` with leftover-memory `inf` values; `clip_grad_norm_` then returns `inf`, `clip_coef` becomes `0`, and `inf * 0 = nan` poisons those LayerNorm params on `optimizer.step()` — `nan` then propagates through the shared trunk to actor and critic alike from epoch 1 onward. Fixed in two parts in `training/ppo/policy.py` (shared by `bc_pretrain.py` and `train.py`/PPO): (1) replaced both `nn.LayerNorm(mlp_hidden)` instances in `CrystalFrontAgent.trunk` with a hand-composed `LayerNorm` (mean/var/normalise via basic ops, avoiding the buggy fused kernel — verified CPU/CUDA agree to ~6 decimals on real captured tensors); (2) even with (1), `clip_grad_norm_` occasionally still returns `inf` from an aggregate-norm overflow with no single non-finite param — added `if not torch.isfinite(grad_norm): optimizer.zero_grad(); continue` to both the actor and critic training loops in `bc_pretrain.py` as a residual skip-guard. Re-ran the full pipeline cleanly: 1000/1000 episodes kept, 265,454 transitions; after `noop_keep_frac=0.05` subsampling, 104,701 transitions (96,241 non-noop + 8,460 noop = 8.1% — well under the "≪90% noop" diagnostic threshold). Actor: 5 epochs, loss 1.2887→0.7907 (monotonic decrease), acc 45.9%→50.6% (peaked 53.2% at epoch 4). Critic: 3 epochs, mse 0.0070→0.0006→0.0005. **Gate results:** BC top-1 accuracy gate (≥55%) **MISSED** at 50.6% final / 53.2% peak. Eval-vs-`idle` gate (≥3/10 via `eval_checkpoint.py`, k=8, greedy) **PASSED overwhelmingly** at 10/10 (100%), avg_ticks=2244, crystal damage 99.6%. A broader 8-opponent eval (10 episodes each, for the record) shows a clean RushBot-clone signature: 100% vs `idle`/`passive`/`rush_weak`, 20% vs `rush_weak_medium`/`macro`, 0% vs `rush_medium`/`rush`/`turtle` — exactly what cloning a pure-rush expert predicts, and a useful pre-Task-3.2 baseline. Per this section's own framing ("if not [≥55%], inspect the action distribution before debugging deeper" — healthy here), and given the eval gate is the operationally decisive check and BC is only a PPO warm-start (Task 3.2 continues training the same network), **accepted as done-with-deviation** rather than spending more GPU time chasing the last ~4 accuracy points. `npm test` 470/470 (no `headless/src` files touched, but env/eval-runner constructor signatures changed — re-verified for regression safety per R6); `test_config_persistence` not re-run — it imports `CrystalFrontVecEnv`, not `CrystalFrontEnv` (the BC/eval path touched here), so it is unaffected. No `BALANCE_HISTORY` entry (no engine/replay-affecting change). Version bumped to `0.5.10-ML` (R1 — training-pipeline + shared network-architecture change). `bc_warmup_v05.pt` (repo root, gitignored) is the Task 3.2 `--checkpoint` input. **Phase 3 Task 3.2 (curriculum run) is next.**

**Audit note (2026-06-12, pre-Task-3.2 implementation audit — docs-only commit, no code changed):** reviewed all completed work (Phases 0–2 + Task 3.1) against this plan before committing to the ~1-week Task 3.2 run. Verdict: the completed work is faithful to spec — every Appendix B ✅ row's verify step was honestly executed and documented — with **three exceptions, all small and all in `training/ppo/train.py`**: (GAP-1) the PPO update loop lacks the `isfinite(grad_norm)` skip-guard Task 3.1 added to `bc_pretrain.py`, a real weight-corruption risk for a week-long run on this ROCm 7.2/torch 2.12.0 GPU — fix before launching Task 3.2; (GAP-2) Task 0.4 step 2 ("have `training/ppo/train.py` print a warning if any episode reports `botCrashCount > 0`") was never implemented — the runner/env plumbing is complete but the trainer-side warning is missing, so row 0.4 is downgraded ✅→⚠️ until it lands; (GAP-3) Task 3.2's realised-config stage-entry logging has no code support (`nextEpisodeConfig` reaches the trainer's info dicts but is never printed), leaving stop-the-line condition 4 unmonitorable without manual effort. Full detail plus run-operator observations (stage `0a` is a near-free win at k=8; stage budgets sum to ~22.75M > the 20M `total_timesteps` cap; list of things verified clean; two untracked working-tree leftovers) are in the new **"Pre-flight findings"** block in §Task 3.2. Two doc corrections were made in the same commit: the header **Status** line (was stale "nothing started") and §Task 3.2's "17-stage" → 20-stage curriculum count. **Recommended path: one `fix(3.2-preflight)` commit closing GAP-1/2/3 (version bump per R1 — training-code change), then launch Task 3.2.**

**Task 3.2, first curriculum run STOPPED (v0.5.11-ML, 2026-06-12, Iteration 21):** launched per the §Task 3.2 command (run `crystalfront_ppo__0_5_11-ML__idle__1__1781254439`). Promoted cleanly through 14 stages (`0a→3a_rwm`, updates 1-66, win rates 57-100%), with `Realised config` matching the stage definition at every promotion (GAP-3 confirmed working on real GPU) and zero `botCrashCount` warnings (GAP-2 confirmed). Promoted to `3a_rm_3k` (update 66) and collapsed to **0.00 win rate for 174 updates** (~890K decisions, `crys_dmg=0%`/`no_pres=100%` throughout), then cascaded through **three regressions** — `3a_rm_3k → 3a_rwm → 3a_rw → 3a` — each landing at 0.00: the *same* stages that had previously scored 61-100% now scored 0%, with `atk_mv`/`tgt` collapsing to 0%. This is **not** stop-the-line condition 2 (`ppo/entropy_bonus` *rose* 0.61→1.87 over the run — the opposite of collapse) nor a KL/clip instability (both stayed in normal ranges); `ppo/value_function_loss` collapsed to ~0.0001, consistent with the critic learning "every episode = −1" with high confidence once the agent stopped winning anywhere. Stopped by user at update 718/step 3675920 (`pkill -f training.ppo.train`, verified dead). **Working hypothesis:** the `3a_rwm` (61% win) → `3a_rm_3k` (`rush_medium`, map 6000→3000px) promotion is too steep a difficulty cliff — ~890K decisions of uniform −1 reward drove catastrophic forgetting of attack behaviour, which then "infected" the regression targets. Full evidence, recovery checkpoint (`checkpoints/crystalfront_ppo__0_5_11-ML__idle__1__1781254439/update_000050.pt`, stage `3a_rw`, win_rate=1.00, pre-`3a_rm_3k`), and open next-steps (inspect `3a_rm_3k`'s definition vs neighbours, cross-check `/root/fable-crystalfront-diagnosis.md`) are in `docs/ML_BOT_ACTION_PLAN.md` Iteration 21. **Root cause not yet identified — do not relaunch Task 3.2 (from `update_000700.pt` or otherwise) until this is investigated.** Task 3.2 remains 🔄.
