...
- **H7 — Bot-decision-layer asymmetry (STILL OPEN — first-pass C.3 #7 audits did not find it).** Since `tick()` (C.1 #14) and `chooseBuildPosition` (C.1 #15) are both now proven mirror-equivariant (mod 0.5px noise), H6's "other asymmetry" must live in code that chooses **when/whether/what** to act, not **where**: `MediumRushBot.step()` (`headless/src/bots/mediumRushBot.ts`), the `buildObservation()` features it reads (`headless/src/observation.ts`), or `resolveTargetZone`/`resolveAttackTarget`/`findNode` (`actionSpace.ts`, used by `attack_move`/`attack_targeted`/`assign_workers`). The 2026-06-10 first-pass audit (C.1 #17-#19) checked `f("blue",s)≈mirrorPos(f("red",s))` on the live (already-RNG-diverged) trajectory — the only on-path violation found (`enemy_crystal`/`nearest_safe` before tick 2128) was clean, and the off-path violations found all trace to RNG-driven state divergence (#18) or a dead code path (#19), not a function-level bug. **A frozen-state, single-color `mirrorState`-based test (à la C.1 #15) does NOT trivially generalize** to `findNode`/`resolveTargetZone`: their `isBlue ? n.x<mid : n.x>mid` filters are *intentionally* side-asymmetric (blue searches its own/left half, red its own/right half), so `f("blue", mirrorState(s)) ≈ mirrorPos(f("blue", s))` is **not** expected to hold even for correct code (mirroring the world while holding color fixed turns "search my own half" into "search the other color's half"). The methodology that remains untried is the **full stateful lockstep test** (C.3 item 7 below): two `MediumRushBot` instances + two parallel match trajectories linked by `mirrorState`, comparing actual chosen actions tick-by-tick. This is the next step.
...
7. ⚠️ **Bot-decision-layer mirror-equivariance test (TOP PRIORITY — see H7) — first pass DONE 2026-06-10, did NOT find H7; full lockstep version still not built.** First pass (`headless/src/_directSymmetryAudit.ts`) checked `f("blue",s)≈mirrorPos(f("red",s))` for `resolveTargetZone`/`findNode` on the live trajectory — see C.1 #17-#19 for the 4 violation classes found and why none of them is conclusively H7 (3 trace to RNG state divergence, 1 is dead code). **The methodology originally specced here, and still needed**: `MediumRushBot` is **stateful** (`lastAssignTick`/`lastPushTick`/`lastDepotTick`/`totalSkirmishersTrained`), so a single-state-snapshot check isn't enough — run TWO bot instances in lockstep from t=0, one driving `s` and one driving a SEPARATE trajectory `m` initialized as `m_0=mirrorState(s_0)` and advanced by its own `engine.tick()` calls (note: `MatchEngine.matches` is private — register `m` via `(engine as any).matches.set(m.id, m)`, or add a small `registerMatch`/`loadMatch` method), and at each tick assert `action(bot_on_m, m, "headless-blue") ≈ mirror(action(bot_on_s, s, "headless-red"))` (and the symmetric pair for red/blue). Because `m_0=mirrorState(s_0)=s_0` (C.1 #13: the start state is self-mirror-symmetric with colors swapped), `m`'s and `s`'s RNG streams start in the SAME swapped relationship that `mirrorState` always produces, so this lockstep should track exactly (mod the accepted 0.5px H4 / spawn-position noise) IF there is no H7 — any growing divergence beyond that noise floor, and the first action where `bot_on_m`'s choice stops being the mirror of `bot_on_s`'s choice, *is* H7. Once found, fix H5+H7 together and re-run the 30-seed matrix (and the rush_weak/macro mirror pairings per C.4) in one pass.
