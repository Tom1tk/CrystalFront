...
- **H7 — Bot-decision-layer asymmetry (NEW, leading hypothesis).** Since `tick()` (C.1 #14) and `chooseBuildPosition` (C.1 #15) are both now proven mirror-equivariant (mod 0.5px noise), H6's "other asymmetry" must live in code that chooses **when/whether/what** to act, not **where**: `MediumRushBot.step()` (`headless/src/bots/mediumRushBot.ts`), the `buildObservation()` features it reads (`headless/src/observation.ts`), or `resolveTargetZone`/`resolveAttackTarget`/`findNode` (`actionSpace.ts`, used by `attack_move`/`attack_targeted`/`assign_workers`). By inspection these all *look* `isBlue`-mirrored or color-independent (e.g. `MediumRushBot`'s `safeNodes` filter `isBlue ? n.xNorm<0.5 : n.xNorm>0.5` against allocentric `xNorm = e.x/mapW` is symmetric; `resolveTargetZone`'s `defend_crystal`/`enemy_crystal` fallbacks use `isBlue ? mapW-100 : 100` correctly) — but none of this has been **property-tested**, and C.1 #10-#15 repeatedly show that "looks symmetric by inspection" is not reliable. C.3 #7 is the next step.
...
7. **Bot-decision-layer mirror-equivariance test (NEW TOP PRIORITY — see H7).** Extend the C.3 #4 harness to `getLegalActions`, `MediumRushBot.step()`, and `resolveTargetZone`/`resolveAttackTarget`/`findNode`. `MediumRushBot` is **stateful** (`lastAssignTick`/`lastPushTick`/`lastDepotTick`/`totalSkirmishersTrained`), so a single-state-snapshot check (like #4's `chooseBuildPosition` test) isn't enough — run TWO bot instances in lockstep from t=0, one driving `s` and one driving `mirrorState(s)` (re-mirrored each tick, or maintained as a parallel mirrored match advanced by its own `engine.tick()`), and at each tick assert `action(mirror-bot, mirrorState(s)) ≈ mirror(action(other-color-bot, s))`. The first action-level divergence (which zone, which target, which timing) *is* H7. Once found, fix H5+H7 together and re-run the 30-seed matrix (and the rush_weak/macro mirror pairings per C.4) in one pass.
