...
**Status:** Phases 0–2 and Task 3.1 are complete (see Appendix B for per-task status and deviation footnotes). Task 3.2's first two curriculum-run attempts (Iterations 21/23, v0.5.11/12-ML) both cascaded to 0% via an entropy-runaway absorbing state that curriculum regression couldn't escape (Iteration 24 diagnosis). Iteration 25 (2026-06-14, v0.5.13-ML) implements both proposed fixes — `ent_coef` 0.02→0.01 and an in-memory stage-entry snapshot rollback on regression — smoke-tested clean on GPU. Relaunch attempt #3 from the Iteration 24 recovery checkpoint is next. *(Original status "Ready to implement — nothing started" superseded by the 2026-06-12 audit.)*
