...
**Last updated:** 2026-06-14 (Phase 3 Task 3.2 — Iteration 26: relaunch #3 STOPPED at update 2231 — Iteration 25 fixes confirmed working (no absorbing collapse, entropy bounded, 3a_rm reached for the first time, 2x), but a new "frontier oscillation" pattern at 3a_rwm/3a_rm emerged; fix proposed (3a_rm pre-placed-skirmisher scaffold), pending sign-off before relaunch #4)
...
- **Relaunch #3 (Iteration 26, 2026-06-14) ran to update 2231 / step 11.17M, STOPPED for a design pause (not a hard stop-condition).** Both Iteration 25 fixes confirmed working: no permanent 0% absorbing collapse (unlike attempts 1/2), `ppo/entropy_bonus` bounded ~0.45-0.62, and **`3a_rm` (Iteration 22's stage) reached for the first time ever, twice** (win 0.63 and 0.66). But a new **"frontier oscillation"** pattern emerged: `3a_rw`↔`3a_rwm` cycled 10/9 times (every regression's rollback recovered `3a_rw` to 89-100%, 9/9), with both `3a_rm` excursions decaying to 0% within ~30-45 updates and rolling back to `3a_rwm` (34%/39%, not 0%) — which then *also* decayed to 0%. ~2150 updates with zero net progress past the `3a_rwm`/`3a_rm` frontier. The literal "3rd cascade" stop condition (rollback → still 0%) did **not** fire (0/9). Per TRAINING_GUIDE §6's ping-pong "note it and ask" guidance, paused for user input — user chose to design a frontier-specific fix. **Proposed (pending sign-off): add `pre_place_units=["skirmisher","skirmisher"]` to `3a_rm`** (idx14), matching the scaffold `3a_rm_3k`/`3a1` already use vs. `rush_medium`, targeting `3a_rm`'s `crys_dmg=0%`/`no_pres=100%` zero-signal symptom. Proposed relaunch #4 checkpoint: `checkpoints/crystalfront_ppo__0_5_13-ML__idle__1__1781439933/update_002000.pt` (stage `3a_rw`, win=0.89, the snapshot that produced this run's 2nd `3a_rm` breakthrough).
...
| Phase 3: retrain + ship v0.5.0-ML | 🔄 In progress — Task 3.1 ⚠️ done-with-deviation (v0.5.10-ML, Iteration 19). Task 3.2: 1st run STOPPED (3x cascade, Iteration 21); curriculum fix landed (v0.5.12-ML, Iteration 22); 2nd run STOPPED again, same cascade one stage earlier (Iteration 23/24); entropy/rollback fixes landed (v0.5.13-ML, Iteration 25, smoke-tested); 3rd run STOPPED for design pause at update 2231 — fixes confirmed working, new "frontier oscillation" at 3a_rwm/3a_rm (Iteration 26); `3a_rm` pre-placed-skirmisher scaffold fix proposed, pending sign-off before relaunch #4, see `docs/REVIVAL_PLAN.md` Appendix B row 3.2 |
...
### 2026-06-14 — v0.5.12-ML Phase 3: Iteration 24 (Task 3.2 relaunch #2 — STOPPED, same cascade now triggers at `3a_rwm` itself; entropy-runaway + no-policy-rollback identified as the deeper mechanism)

**What happened:** The Iteration 23 relaunch (fresh from `bc_warmup_v05.pt`, fixed 21-stage curriculum) promoted cleanly through `0a→3a_rw` (updates 1-55; e.g. `2b` 94%, `3a` 99%, `3a_rw` 100% — matching/exceeding Iteration 21). On entering **`3a_rwm`** (idx13, step 281600), the FIRST evaluation window scored **win_rate=0.13 (13/100)**, `tmt=0.86` — far below Iteration 21's 61% on the same stage. The next window (step 427460) scored **0.00** and stayed at 0.00 for the rest of `3a_rwm`'s `max_steps=1,000,000` budget (~190 updates, `crys_dmg=0% no_pres=100%` throughout). At step 1,295,360 the curriculum REGRESSED to `3a_rw` (idx12) — the same stage that scored 100% ~150 updates earlier — which then ALSO scored 0.00 for 8 consecutive windows (steps 1,359,360-1,705,860). Stopped (`pkill -f training.ppo.train`) per the Step-3 cascading-regression criterion.

**Key new finding: the run never reached the new `3a_rm` stage (idx14) — Iteration 22's fix was not exercised.** The collapse happened one stage earlier, at `3a_rwm` itself — a **single-variable** transition from `3a_rw` (only `opponent` changes, `rush_weak`→`rush_weak_medium`; same 6000px map/HP/resources). A clean single-variable transition reproduced the identical absorbing-collapse signature. This is strong evidence that **"4 variables at once" (Iteration 22's framing) is a real R3 violation worth keeping fixed, but is not the root cause** of the collapse itself.

**Refined diagnosis — entropy runaway is an absorbing state, largely independent of curriculum stage:**

- `ppo/entropy_bonus` (fixed `ent_coef=0.02`, no annealing — `train.py` line 193/881) has been on a **slow monotonic uptrend since stage ~7-8** (step 56320: 0.33 → step 209920: 0.78, while those stages scored 90-100%+ — entropy rises even while winning). It accelerates through stages 12-13 (0.78→1.0→1.2→1.5→2.1 by step ~425000) and then **plateaus at ~1.4-2.0 indefinitely, including after regressing back to stage 12**, which never recovers its earlier ~0.6-0.8 entropy or its earlier 100% win rate.
- `ppo/value_function_loss` stayed small (0.0007-0.024) throughout — not as dramatically flat as Iteration 21's `→0.0001`, but consistently small/uninformative, consistent with a near-constant value estimate once most outcomes are losses.
- **Mechanism:** as pg_loss/value gradients shrink (either from "already winning easily" on early stages, or "always losing" on `3a_rwm`), the fixed `ent_coef * entropy` term becomes a relatively larger share of the loss gradient — and increasing entropy is a "free" way to reduce loss, so the optimizer pushes entropy up. Once entropy crosses roughly ~1.0-1.5, the policy is too noisy to reliably execute the multi-step build→train→attack_move→target chains needed to win — win_rate craters to 0% on ANY stage (easy or hard), `no_pres=100%`, removing the last gradient signal and entrenching high entropy. **This is an absorbing state**: more training time doesn't escape it, and curriculum regression doesn't either — regression only changes `cur_stage_idx`/env difficulty (`train.py` lines 960-982, regression branch only rebuilds `vec_envs`), it does **not** reload `agent`/`optimizer` state. The same degraded, high-entropy policy is simply pointed at an easier opponent and still fails.

**Why two runs diverged so much by `3a_rwm` (61% vs 13% first-window win rate):** PPO is highly stochastic (env reset seeds, minibatch shuffling, rollout sampling) — by update ~55-70, the two runs are different policies despite an identical starting checkpoint and command. Whether the first `3a_rwm` window lands above or below the 50% promotion threshold looks close to a coin flip; landing below it triggers the entropy-runaway absorbing state, which then cascades regardless of which `3a_*` stage it started from. **This reframes the problem from "which stages should be combined/split" (Iteration 22) to "what stops the entropy-runaway absorbing state once a stage's first window is unlucky."**

**What would NOT have prevented this run's failure:** Iteration 22's `3a_rm` insertion and `max_steps` cuts — both remain valid R3-compliant improvements and stay in the curriculum, but neither addresses the entropy/no-rollback mechanism above, and neither was exercised by this run.

**Proposed fix (pending Checkpoint sign-off, do NOT relaunch yet):**

1. **Reduce/anneal `ent_coef`** (currently fixed `0.02`, `train.py` line 193) — directly targets the runaway. Either a flat reduction (e.g. 0.02→0.01) or linear annealing toward a floor over the run (mirroring the existing `--anneal_lr` mechanism), so exploration pressure decreases as training progresses and a runaway can't sustain itself indefinitely.
2. **Checkpoint rollback on curriculum regression** — when `pending_stage_change == -1` ("stuck"), in addition to `cur_stage_idx -= 1`, reload `agent`/`optimizer` state from the most recent checkpoint saved at-or-before the failing stage's entry, before resuming on the easier stage. Directly fixes "regression doesn't recover because the POLICY broke, not the environment" — restoring the known-good pre-collapse policy gives the easier stage a policy that can win it again, breaking the cascade.

Both are evidence-driven by the same entropy/value-loss data from two independent runs and address complementary halves of the mechanism (1: slow/prevent the runaway; 2: recover if it happens anyway).
...
### 2026-06-14 — v0.5.13-ML Phase 3: Iteration 26 (Task 3.2 relaunch #3 — STOPPED at update 2231, both Iteration 25 fixes confirmed working, new "frontier oscillation" pattern at 3a_rwm/3a_rm; fix proposed, pending sign-off)

**What happened:** relaunch #3 (resumed from `update_000050.pt`, `--curriculum_stage 12`, run `crystalfront_ppo__0_5_13-ML__idle__1__1781439933`, log `train_task32_v3.log`) ran to update 2231 / step 11,166,420 before being stopped (`pkill -f training.ppo.train`) for a design-decision pause — **not** because either Iteration-24 stop condition (entropy runaway / no-rollback) recurred, but because a new, well-evidenced pattern emerged that TRAINING_GUIDE §6's "ping-ponging between stages → note it and ask" guidance covers.

**Both Iteration 25 fixes are CONFIRMED WORKING for their core purpose:**

- `ppo/entropy_bonus` stayed **bounded (~0.45-0.62)** for the back half of the run, vs. 1.13-1.45 mid-run and the unbounded creep to 1.4-2.0+ in attempts 1/2. Rollback visibly resets entropy.
- **No permanent 0% absorbing collapse occurred** — the single biggest difference from attempts 1/2, both of which ended in an unrecoverable 0% stop.
- `3a_rw`→`3a_rwm` promoted **10x**; `3a_rwm`→`3a_rw` regressed **9x**, each preceded by `*** ROLLBACK → restored agent/optimizer to entry-state of stage 3a_rwm ***`. **9/9 post-rollback `3a_rw` windows recovered to 89-100%** within ~8 updates — the rollback mechanism works exactly as designed for this pair.
- **`3a_rm` (Iteration 22's stage) was reached for the first time ever across all 3 attempts — twice** (update 1726, win 0.63; update 2013, win 0.66).

**New finding — "frontier oscillation" at the `3a_rwm`/`3a_rm` boundary:**

Both `3a_rm` attempts then declined to 0.00 within 30-45 updates (1st: 0.07→0.00 over updates 1741-1792; 2nd: 0.01→0.00 over updates 2028-2073) and regressed back to `3a_rwm` (`*** ROLLBACK → restored agent/optimizer to entry-state of stage 3a_rm ***`, restoring `3a_rwm` at 34%/39% — **not** 0%). Both of *those* `3a_rwm` recoveries then *also* decayed to 0.00 (1st: 0.34→0.00 over updates 1805-1994, regressing to `3a_rw` for the 9th — and successful — recovery; 2nd: 0.39→0.00 over updates 2087-2146, and remained at **0.00 for 6 consecutive windows** through update 2231, with `atk_mv` collapsing 24%→1% and `noop` climbing 42%→62%, the same "noop-heavy degenerate policy" signature that preceded all 9 prior `3a_rwm`→`3a_rw` regressions — a 10th was likely imminent, `stage_steps`≈740K/1,000,000, when stopped).

Net effect: **~2150 updates (~10.6M steps) since first reaching `3a_rwm`, with zero net curriculum progress past the `3a_rwm`/`3a_rm` frontier** — a stable `3a_rw`↔`3a_rwm` oscillation (10/9 cycles, every regression recovering) with rare (2/10), so-far-unsuccessful excursions into `3a_rm`.

**Why this did NOT trigger the literal "3rd cascade → STOP+pkill+escalate" stop condition** (from the Step-3 monitoring brief, memory `project-task32-goal-2026-06-14`): that condition was "regression → rollback → STILL 0% on `3a_rw` or `3a_rwm` *again*" — i.e. the rollback itself fails to recover anything, repeating attempts 1/2's unrecoverable-collapse signature. That has occurred **0/9 times**: every `3a_rw` rollback recovered to 89-100%, and both `3a_rwm` rollbacks started at 34-39% (not 0%). What's happening instead is a *different* failure mode — a recoverable but unproductive oscillation — which is why this was raised as a "note it and ask" pause rather than an automatic stop.

**User decision (2026-06-14):** presented both the positive results and the frontier-oscillation finding; user chose **"Pause + design a frontier fix"** — stop the run, write this diary entry, and propose a targeted fix for the `3a_rwm`→`3a_rm` transition for sign-off before relaunch #4 (Checkpoint-1-style — do not implement/relaunch without approval).

**Proposed fix for Iteration 27 (PENDING SIGN-OFF — not yet implemented):**

- **Root-cause candidate:** every `3a_rm` attempt (opponent=`rush_medium`, idx14, `map_width=0`/default, no pre-placed units, `starting_resources=200`) scores `crys_dmg=0%`/`no_pres=100%` throughout — *literally zero combat signal*. The agent enters with 0 units against a 6-unit `rush_medium` opponent and appears to be overrun/never establishes presence before it can train anything, regardless of how well it had just been playing `3a_rwm` (63-66% win rate at the moment of promotion).
- **Proposed single-variable change:** add `pre_place_units=["skirmisher", "skirmisher"]` to stage `3a_rm` (idx14) — the *exact* scaffold that `3a_rm_3k` (idx15) and `3a1` (idx16) already use against this same `rush_medium` opponent. This gives the agent initial combat capability so it can contest `rush_medium` and produce non-zero `crys_dmg`/signal, turning `3a_rm` into a test of "can the policy learn to beat `rush_medium` at all" rather than "can it survive `rush_medium` from zero units on a 6000px map" — the latter being a much harder, and per the matrix data (`rush_weak_medium` 0/50 vs `rush_medium`), possibly near-impossible ask in 300K steps. `promotion_threshold=0.50` and `max_steps=300_000` stay unchanged — exactly one field changes.
- **Alternative considered, not chosen:** raise `3a_rwm`'s `promotion_threshold` (0.50→0.65+) so promotion to `3a_rm` only fires from a more dominant position. Rejected — both observed promotions (0.63, 0.66) were already comfortably above 0.50, so this doesn't look like "promoted on a noisy near-threshold peak"; it looks like a categorical opponent-tier transfer gap that a higher `3a_rwm` bar wouldn't close, while making `3a_rwm` itself harder to clear (compounding the existing oscillation).
- **What this fix does *not* address directly:** the `3a_rw`↔`3a_rwm` oscillation itself is not separately touched. If the scaffold lets `3a_rm` sustain non-zero/growing signal and eventually clear (≥0.50), the oscillation resolves naturally (curriculum advances past idx14, no more regressions back through `3a_rwm`/`3a_rw`). If `3a_rm` *still* shows `crys_dmg=0%` even with 2 pre-placed skirmishers, that's new evidence the wall is `max_steps`/opponent-tier itself rather than zero-army — a candidate single-variable follow-up for a later iteration (not bundled here, per R3).
- **Proposed relaunch #4 checkpoint:** `checkpoints/crystalfront_ppo__0_5_13-ML__idle__1__1781439933/update_002000.pt` — saved immediately after the 9th `3a_rwm`→`3a_rw` ROLLBACK+REGRESS (the restored `3a_rwm`-entry-snapshot policy, freshly placed into `3a_rw`), which in *this* run went on to score 0.89→promote→0.66 on `3a_rwm`→promote to `3a_rm` (2nd breakthrough). A known-good, recent, well-characterized starting point — avoids resuming from the currently-degenerate (`noop`=62%, `atk_mv`=1%) `update_002200.pt` policy. Command (for reference, pending approval):
  ```bash
  python3 -m training.ppo.train --curriculum --curriculum_stage 12 \
    --checkpoint checkpoints/crystalfront_ppo__0_5_13-ML__idle__1__1781439933/update_002000.pt \
    --decision_interval 8 --num_envs 20 --vec_size 4 --total_timesteps 20000000
  ```

**No version bump** — this entry documents attempt-3 results and a proposed fix only; no code has changed (R1 n/a for this entry).
