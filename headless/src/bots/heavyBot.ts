 ...
 */
export class HeavyBot implements Agent {
  private playerId = "";
  ...
  private turretYZones: MacroAction["yZone"][] = ["middle", "top", "bottom"];
  private turretBaseZone = 0;
  ...
  step(obs: PlayerObservation, legal: MacroAction[]): MacroAction[] {
    ...
    // Phase transitions
    if (this.phase === "eco"  && hasFoundry)     this.phase = "tech";
    if (this.phase === "tech" && armySize >= 4)  this.phase = "mass";
    if (this.phase === "mass" && armySize >= 8)  this.phase = "push";
