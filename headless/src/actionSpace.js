...
export function expandMacroAction(action, match, playerId) {
    ...
    switch (action.type) {
        ...
        case "build": {
            ...
            if (workers.length === 0)
                return [];
            const ownBuildingCount = [...match.entities.values()].filter(e => e.ownerId === playerId && e.type === "building").length;
            const Y_CYCLE = ["middle", "top", "bottom"];
        ...
        case "attack_targeted": {
            ...
            if (targetType === "spread_fire") {
                ...
                const enemyCombat = visibleEnemies.filter(e => ["skirmisher", "gunner", "bruiser", "medic"].includes(e.type));
...
function getIdleWorkers(match, playerId) {
    const workers = [];
    for (const e of match.entities.values()) {
...
function getAvailableBuilders(match, playerId) {
    const workers = [];
    for (const e of match.entities.values()) {
        if (e.type !== "worker" || e.ownerId !== playerId)
            continue;
...
function getUnitGroup(match, playerId, group) {
    ...
    for (const e of match.entities.values()) {
        ...
        if (group === "all_idle_combat" && isCombat && isIdle) {
            results.push(e);
            continue;
        }
        if (group === "idle_workers" && e.type === "worker" && isIdle) {
            results.push(e);
            continue;
        }
    }
...
function getVisibleEnemies(match, playerId, visibleIds) {
    const enemies = [];
    for (const e of match.entities.values()) {
        if (e.ownerId === playerId)
            continue;
 ...
 */
function resolveAttackTarget(unit, visibleEnemies, targetType, playerColor, match) {
...
function chooseBuildPosition(color, xZone, yZone, match) {
    ...
    const ownCrystals = blockingEntities.filter(e => e.type === "crystal" && e.ownerId === (isBlue ? match.players[0]?.playerId : match.players[1]?.playerId));
    for (let attempt = 0; attempt < 16; attempt++) {
        ...
        if (!entityBlocked && !crystalBlocked)
            return { x: cx, y: cy };
    }
    return null; // no valid position found — caller treats as illegal
}
function resolveTargetZone(zone, playerColor, match, visibleIds) {
    const isBlue = playerColor === "blue";
    ...
    if (zone === "enemy_crystal") {
        ...
        return { x: isBlue ? mapWidth - 100 : 100, y: mapHeight / 2 };
    }
    if (zone === "midfield") {
        return { x: mid, y: mapHeight / 2 };
    }
    if (zone === "contested_node") {
        let nearest = { x: mid, y: mapHeight / 2 };
        let nearestDist = Infinity;
        for (const node of match.resourceNodes) {
            if (Math.abs(node.x - mid) < mapWidth * 0.35) {
    ...
    if (zone === "enemy_army") {
        ...
        const enemyCombat = [...match.entities.values()].filter(e => e.ownerId !== (match.players.find(p => p?.color === playerColor)?.playerId ?? "") &&
            visibleIds.has(e.id) &&
    ...
    if (zone === "defend_crystal") {
        ...
        const cristalX = ownCrystal?.x ?? (isBlue ? 100 : mapWidth - 100);
        const crystalY = ownCrystal?.y ?? mapHeight / 2;
        const threats = [...match.entities.values()].filter(e => e.ownerId !== ownPlayerId && visibleIds.has(e.id) &&
            ["skirmisher", "gunner", "bruiser", "medic"].includes(e.type));
        if (threats.length > 0) {
            // Find nearest threat to own crystal
            const nearest = threats.sort((a, b) => dist2(a, { x: cristalX, y: crystalY }) - dist2(b, { x: cristalX, y: crystalY }))[0];
...
function findNode(match, playerId, choice, nearEntity) {
    ...
    if (choice === "nearest_contested") {
        const cont = availableNodes.filter(n => Math.abs(n.x - mid) < mapWidth * 0.35);
