...
import { BUILDING_DEFS, UNIT_DEFS, COUNTER_MULTIPLIERS, } from "@crystalfront/shared";
import { GATHERING, NODES, SIMULATION, HEALING, ECONOMY, } from "@crystalfront/shared";
...
export class MatchEngine {
    ...
    computeVisibility(match) {
        ...
        for (const [playerId, vis] of visibility) {
            const myEntities = allEntities.filter((e) => e.ownerId === playerId &&
    ...
    buildSpatialGrid(entities, cellSize) {
        const grid = new Map();
    ...
    processMovement(match, subStepMs = 100) {
                    ...
                    for (let ddy = -1; ddy <= 1; ddy++) {
                        ...
                        for (const other of cell) {
                            if (other.id <= entity.id)
                                continue;
                            if (checked.has(`${entity.id}-${other.id}`))
                                continue;
    ...
    getCounterMultiplier(attackerType, defenderType) {
    ...
    processGathering(match) {
        for (const node of match.resourceNodes) {
            ...
            if (wholeResources > 0) {
                ...
                const gatheredByPlayer = new Map();
                for (const workerId of inRangeWorkers) {
                    const worker = match.entities.get(workerId);
                    if (worker) {
                        gatheredByPlayer.set(worker.ownerId, (gatheredByPlayer.get(worker.ownerId) ?? 0) + perWorker);
                    }
                }
                let distributed = 0;
        ...
        for (const entity of match.entities.values()) {
            ...
            if (entity.type === "medic" && entity.healTargetId) {
                ...
                if (d <= healRange + target.radius && target.health < target.maxHealth) {
                    const healed = Math.min(target.maxHealth, target.health + HEALING.healRatePerTick);
