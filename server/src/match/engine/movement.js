...
export function processMovement(match, subStepMs = 100) {
    ...
    for (const entity of entities) {
        ...
        const speedPerTick = entity.type === "worker" && match.config.workerSpeed !== undefined ? match.config.workerSpeed :
            entity.type === "skirmisher" && match.config.skirmisherSpeed !== undefined ? match.config.skirmisherSpeed :
                baseSpeed;
        const moveAmount = Math.min(speedPerTick / stepsPerTick, dist(entity.x, entity.y, entity.moveTarget.x, entity.moveTarget.y));
        ...
        for (const other of entities) {
            if (other.id === entity.id)
                continue;
            if (other.type !== "crystal" && other.type !== "building")
                continue;
